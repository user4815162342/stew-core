...
var DirectoryIndex = module.exports.DirectoryIndex = function(prototype,ContentItem,dontSort) {
     ...
     * match the RegEx pattern. Certain RegEx's (TODO: Which ones) will
     * allow an attempt to match recursively through child directories.
    ...
    prototype.matchDocs = function(pattern,cb) {
        ...
        if (typeof pattern === "string") {
            ...
            this._project.root().matchDocs(pattern,cb);
        }
        ...
        if (pattern instanceof minimatch.Minimatch) {
            ...
            asyncForEach(patternSet,function(pattern,cb) {
            ...
            if (typeof patternItem === "string") {
                ...
                this.getDoc(patternItem,function(err,doc) {
                    if (err) {
                ...
                this.matchDocs(patternItem,function(err,docs) {
                    if (err) {
                        return cb(err);
                    }
                    if (pattern.length === 0) {
